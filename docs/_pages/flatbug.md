---
title: PhotoDB Example with Flatbug Dataset
sidebar:
  nav: flatbug
---
...
On the [GitHub repository](https://github.com/envima/photodb/tree/main/photodb_reproducible_example_flatbug) you can find a reprodicble example PhotoDB instance using the flatbug dataset.  
**Prerequisite: Java 17 (or newer) installation**

## Setup the Flatbug PhotoDB Instance

**1. Download and unpack the ``database.zip`` file. This is your PhotoDB instance.**
...
After initial metadata creation, the flatbug detections were then integrated into the metadata using the [``flatbug_json_to_yaml.ipynb``](https://github.com/envima/photodb/blob/main/photodb_reproducible_example_flatbug/flatbug_json_to_yaml.ipynb) notebook. See 5. for a detailed explanation.

#### ``config.yaml``

This file contains the configuration of this PhotoDB instance, e.g., data / metadata storage location, project name, etc. (see [configuration](/photodb/configuration/PhotoDB.html)).

#### ``photo_classification_definitions.csv``

This file contains annotation classes used in the web interface PhotoApp (see [classification definition](/photodb/usage/classification_definition.html)).
...
Open the web interface 'PhotoApp' at http://127.0.0.1:8080/ and click <code>Go to AudioApp</code>. This is necessary even though we are using PhotoApp, as all tasks are handled in AudioApp. Now click on the top left button to open the left navigation side panel and select the entry <code>Task submission</code>.

<br><br>

Select the task <code>photo_create_yaml</code> from the drop down menu at the top left, optionally provide the project name (flatbug_dataset), and click <code>submit task</code>. A pop-up window will open showing you the progress of the task and indicate when the task is done.

<br><br>

<img src="/photodb/assets/flatbug_photo_create_yaml.png" alt="PhotoDB AudioDB task submission" width="auto" height="300" align="center">

<br><br>

Your metadata files will now be saved in the directory specified as <code>photo_meta</code> in the <code>config.yaml</code> file.
...
Once YAML metadata files are instantiated you can easily manipulate them via direct access, e.g., using a processing script. We used the <a href="https://github.com/envima/photodb/blob/main/photodb_reproducible_example_flatbug/flatbug_json_to_yaml.ipynb"><code>flatbug_json_to_yaml.ipynb</code></a> notebook provided on GitHub, which automates the process of merging JSON metadata from the Flatbug dataset into an existing collection of PhotoDB YAML files as following:
...
Open the web interface in a browser at http://127.0.0.1:8080/ (see [operating](/photodb/operating/operating_windows.html)) and click ``Go to PhotoApp``.
...
<img src="/photodb/assets/flatbug_annotations.png" alt="PhotoDB AudioDB task submission" width="auto" height="300" align="center">

See [viewer](/photodb/usage/photoapp_viewer.html) for detailed explanations of the controls.